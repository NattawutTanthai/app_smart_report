...
function TaskCard({ screen, task }) {
  ...
  const { imgStart, endDate_timeStamp, startDate_timeStamp, processDate_timeStamp, _id, detail, name } = task.task;
  // Day.js
  dayjs.locale('th');
        ...
        <View className="flex-col flex-wrap justify-around ml-2">
          {
            screen == 'DetailWaitReport' ? (
              <Text className="text-[#FFBD00] font-kanitRegular">
                สถานะ : รอรับเรื่องมา {dayjs().to(dayjs(startDate_timeStamp))}
              </Text>
